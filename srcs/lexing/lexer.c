...
/*   Updated: 2024/06/21 09:19:44 by isb3             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	split_array_utils(char ***array, char **tokens, int *i, int *k)
{
	...
	array[*k] = m_malloc((len + 1) * sizeof(char *));
	if (!array[*k])
		return (free(array), 1);
	array[*k][len] = NULL;
	j = 0;
	while (tokens[*i] && tokens[*i][0] != '|')
	{
		...
		if (!array[*k][j])
			return (free_array_of_array(array), 1);
		j++;
		(*i)++;
	}
	if (tokens[*i])
...
char	***split_array(char ***array, char **tokens, int i, int k)
{
	...
	array = m_malloc((len + 2) * sizeof(char **));
	if (!array)
		return (NULL);
	array[len + 1] = NULL;
	while (tokens[i])
...
int	lexer_utils(char ****array, char **tokens)
{
	...
	*array = m_malloc(2 * sizeof(char **));
	if (!*array)
		return (1);
	(*array)[0] = m_malloc((len + 1) * sizeof(char *));
	if (!(*array)[0])
		return (free(array), 1);
	(*array)[0][len] = NULL;
	i = -1;
...
char	***lexer(char *s)
{
	...
	len = count_rows(s, 0);
	tokens = m_malloc((len + 1) * sizeof(char *));
	if (!tokens)
		return (free(s), NULL);
	tokens[len] = NULL;
